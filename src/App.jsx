...
.nav{padding:12px 9px;flex:1;overflow-y:auto;display:flex;flex-direction:column}
...
/* ── LANDING PAGE ── */
.lp{background:var(--bg);color:var(--t1);font-family:'DM Sans',sans-serif;overflow-x:hidden;min-height:100vh}
.lp *{box-sizing:border-box}
.lp-grain{position:fixed;inset:0;background-image:url("data:image/svg+xml,%3Csvg viewBox='0 0 512 512' xmlns='http://www.w3.org/2000/svg'%3E%3Cfilter id='n'%3E%3CfeTurbulence type='fractalNoise' baseFrequency='0.75' numOctaves='4' stitchTiles='stitch'/%3E%3C/filter%3E%3Crect width='100%25' height='100%25' filter='url(%23n)' opacity='1'/%3E%3C/svg%3E");opacity:.035;pointer-events:none;z-index:1000}
.lp-nav{position:fixed;top:0;left:0;right:0;z-index:900;display:flex;align-items:center;justify-content:space-between;padding:18px 48px;transition:all .3s}
.lp-nav.sc{background:rgba(10,8,16,.97);border-bottom:1px solid rgba(212,168,67,.12);backdrop-filter:blur(12px)}
.lp-nav-logo{display:flex;align-items:center;gap:10px;text-decoration:none;cursor:pointer}
.lp-nav-icon{width:36px;height:36px;background:linear-gradient(135deg,var(--gold),var(--goldd));border-radius:8px;display:flex;align-items:center;justify-content:center;font-size:18px}
.lp-nav-text{font-family:'Playfair Display',serif;font-size:20px;font-weight:700;color:var(--gold)}
.lp-nav-links{display:flex;align-items:center;gap:28px;list-style:none}
.lp-nav-links a{color:var(--t2);text-decoration:none;font-size:13px;font-weight:500;letter-spacing:.03em;transition:color .2s;cursor:pointer}
.lp-nav-links a:hover{color:var(--t1)}
.lp-cta-btn{background:linear-gradient(135deg,var(--gold),var(--goldd));color:#1a1000;padding:8px 20px;border-radius:6px;font-weight:600!important;border:none;cursor:pointer;font-family:'DM Sans',sans-serif;font-size:13px;transition:filter .2s,transform .2s}
.lp-cta-btn:hover{filter:brightness(1.1);transform:translateY(-1px)}
.lp-hero{min-height:100vh;position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;padding:120px 24px 80px;overflow:hidden}
.lp-hero::after{content:'';position:absolute;top:-20%;left:50%;transform:translateX(-50%);width:900px;height:900px;background:radial-gradient(ellipse at center,rgba(212,168,67,.09) 0%,transparent 70%);pointer-events:none}
.lp-curtain-l,.lp-curtain-r{position:absolute;top:0;bottom:0;width:180px;background:linear-gradient(to right,rgba(80,10,10,.35),transparent);pointer-events:none}
.lp-curtain-r{right:0;left:auto;background:linear-gradient(to left,rgba(80,10,10,.35),transparent)}
.lp-stage-line{position:absolute;bottom:0;left:0;right:0;height:2px;background:linear-gradient(to right,transparent,rgba(212,168,67,.4),transparent)}
.lp-eyebrow{font-size:10px;font-weight:600;letter-spacing:4px;text-transform:uppercase;color:var(--gold);margin-bottom:24px;display:flex;align-items:center;gap:12px;animation:fi .8s ease .2s both}
.lp-eyebrow::before,.lp-eyebrow::after{content:'';width:40px;height:1px;background:var(--goldd);opacity:.6}
.lp-h1{font-family:'Playfair Display',serif;font-size:clamp(52px,8vw,108px);font-weight:900;line-height:.92;letter-spacing:-.02em;margin-bottom:12px;animation:lp-up .9s ease .35s both}
.lp-h1 em{font-style:italic;color:var(--gold);display:block}
.lp-h1 span{display:block;-webkit-text-stroke:1px rgba(212,168,67,.3);color:transparent}
.lp-sub{font-family:'Playfair Display',serif;font-style:italic;font-size:clamp(17px,2.2vw,24px);color:var(--t2);max-width:540px;line-height:1.5;margin:24px auto 44px;animation:lp-up .9s ease .5s both}
.lp-btns{display:flex;gap:14px;justify-content:center;flex-wrap:wrap;animation:lp-up .9s ease .65s both}
.lp-btn-p{display:inline-flex;align-items:center;gap:8px;padding:15px 32px;background:linear-gradient(135deg,var(--gold),var(--goldd));color:#1a1000;border:none;border-radius:6px;font-family:'DM Sans',sans-serif;font-size:15px;font-weight:700;cursor:pointer;letter-spacing:.02em;transition:filter .2s,transform .2s,box-shadow .2s;box-shadow:0 0 40px rgba(212,168,67,.2)}
.lp-btn-p:hover{filter:brightness(1.1);transform:translateY(-2px);box-shadow:0 4px 50px rgba(212,168,67,.35)}
.lp-btn-s{display:inline-flex;align-items:center;gap:8px;padding:15px 32px;background:transparent;color:var(--t1);border:1px solid rgba(212,168,67,.25);border-radius:6px;font-family:'DM Sans',sans-serif;font-size:15px;font-weight:500;cursor:pointer;transition:border-color .2s,background .2s}
.lp-btn-s:hover{border-color:rgba(212,168,67,.5);background:rgba(212,168,67,.05)}
.lp-stats{display:flex;gap:48px;margin-top:72px;animation:lp-up .9s ease .8s both;flex-wrap:wrap;justify-content:center}
.lp-stat-n{font-family:'Playfair Display',serif;font-size:36px;font-weight:700;color:var(--gold);line-height:1}
.lp-stat-l{font-size:11px;color:var(--t3);letter-spacing:2px;text-transform:uppercase;margin-top:4px}
.lp-scroll{position:absolute;bottom:32px;left:50%;transform:translateX(-50%);display:flex;flex-direction:column;align-items:center;gap:6px;color:var(--t3);font-size:10px;letter-spacing:2px;text-transform:uppercase;animation:bob 2s ease-in-out infinite}
.lp-sec{padding:100px 24px;position:relative}
.lp-sec.dark{background:var(--bg2);border-top:1px solid var(--bd);border-bottom:1px solid var(--bd)}
.lp-con{max-width:1100px;margin:0 auto}
.lp-slbl{font-size:10px;letter-spacing:4px;text-transform:uppercase;color:var(--goldd);font-weight:600;margin-bottom:12px}
.lp-stitle{font-family:'Playfair Display',serif;font-size:clamp(28px,4vw,52px);font-weight:700;line-height:1.1;margin-bottom:16px}
.lp-ssub{font-family:'Playfair Display',serif;font-style:italic;font-size:19px;color:var(--t2);max-width:560px;line-height:1.5}
.lp-prob-grid{display:grid;grid-template-columns:1fr 1fr 1fr;gap:2px;margin-top:48px;background:var(--bd);border:1px solid var(--bd);border-radius:12px;overflow:hidden}
.lp-prob-card{background:var(--bg2);padding:36px 28px;position:relative}
.lp-prob-n{font-family:'Playfair Display',serif;font-size:64px;font-weight:900;color:rgba(212,168,67,.07);position:absolute;top:16px;right:20px;line-height:1;user-select:none}
.lp-prob-icon{font-size:28px;margin-bottom:14px}
.lp-prob-title{font-family:'Playfair Display',serif;font-size:18px;margin-bottom:8px}
.lp-prob-desc{font-size:13px;color:var(--t2);line-height:1.6}
.lp-feat-row{display:grid;grid-template-columns:1fr 1fr;gap:80px;align-items:center;margin-bottom:100px}
.lp-feat-vis{border-radius:14px;overflow:hidden;border:1px solid var(--bd);background:var(--bg3);position:relative;min-height:320px;display:flex;align-items:center;justify-content:center}
.lp-feat-vis::before{content:'';position:absolute;inset:0;background:linear-gradient(135deg,rgba(212,168,67,.05),transparent 60%)}
.lp-feat-n{font-family:'Playfair Display',serif;font-size:72px;font-weight:900;color:rgba(212,168,67,.08);line-height:1;margin-bottom:-8px}
.lp-feat-title{font-family:'Playfair Display',serif;font-size:34px;font-weight:700;line-height:1.15;margin-bottom:16px}
.lp-feat-desc{font-size:15px;color:var(--t2);line-height:1.7;margin-bottom:24px}
.lp-feat-ul{list-style:none;display:flex;flex-direction:column;gap:8px}
.lp-feat-ul li{display:flex;align-items:flex-start;gap:10px;font-size:13.5px;color:var(--t2)}
.lp-feat-ul li::before{content:'◆';color:var(--goldd);font-size:8px;margin-top:5px;flex-shrink:0}
.lp-cat-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(155px,1fr));gap:12px;margin-top:48px}
.lp-cat-card{background:var(--bg3);border:1px solid var(--bd);border-radius:10px;padding:20px 16px;text-align:center;transition:border-color .2s,transform .2s,background .2s}
.lp-cat-card:hover{border-color:rgba(212,168,67,.3);background:rgba(212,168,67,.03);transform:translateY(-3px)}
.lp-testi-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:20px;margin-top:48px}
.lp-testi-card{background:var(--bg3);border:1px solid var(--bd);border-radius:12px;padding:28px;position:relative}
.lp-testi-card::before{content:'\201C';font-family:'Playfair Display',serif;font-size:72px;color:rgba(212,168,67,.15);position:absolute;top:10px;left:20px;line-height:1}
.lp-testi-text{font-family:'Playfair Display',serif;font-style:italic;font-size:17px;color:var(--t1);line-height:1.6;margin-bottom:20px;position:relative;z-index:1;padding-top:20px}
.lp-cta-sec{background:var(--bg2);border-top:1px solid rgba(212,168,67,.1);text-align:center;padding:120px 24px;position:relative;overflow:hidden}
.lp-cta-glow{position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);width:700px;height:700px;background:radial-gradient(ellipse,rgba(212,168,67,.07),transparent 70%);pointer-events:none}
.lp-footer{background:var(--bg);border-top:1px solid var(--bd);padding:48px 48px 32px}
.lp-foot-inner{max-width:1100px;margin:0 auto;display:flex;justify-content:space-between;align-items:flex-start;flex-wrap:wrap;gap:32px;margin-bottom:40px}
.lp-foot-bottom{max-width:1100px;margin:0 auto;display:flex;justify-content:space-between;align-items:center;padding-top:24px;border-top:1px solid var(--bd);flex-wrap:wrap;gap:12px}
...
/* Curtain transition */
.curtain-trans{position:fixed;inset:0;z-index:9999;pointer-events:none;display:flex}
.ct-left,.ct-right{flex:1;background:linear-gradient(to bottom,#1a0a00,#0d0b11);transition:transform .7s cubic-bezier(.77,0,.18,1)}
.ct-left{transform-origin:left;border-right:2px solid rgba(212,168,67,.4)}
.ct-right{transform-origin:right;border-left:2px solid rgba(212,168,67,.4)}
.curtain-trans.open .ct-left{transform:translateX(-100%)}
.curtain-trans.open .ct-right{transform:translateX(100%)}
.curtain-trans.hidden{display:none}

/* Back to landing pill in sidebar */
...
@media(max-width:900px){.lp-nav{padding:16px 20px}.lp-nav-links{display:none}.lp-feat-row{grid-template-columns:1fr;gap:32px}.lp-prob-grid{grid-template-columns:1fr}.lp-testi-grid{grid-template-columns:1fr}.lp-curtain-l,.lp-curtain-r{width:60px}.lp-footer{padding:40px 20px 24px}.lp-sec{padding:72px 20px}}
@media(max-width:600px){.lp-cat-grid{grid-template-columns:repeat(3,1fr)}.lp-btns{flex-direction:column;align-items:center}}
`;
...
export default function App(){
  ...
  // 'landing' | 'curtain-closing' | 'app' | 'curtain-returning' 
  const[scene,setScene]=useState("landing");
  const[curtainPhase,setCurtainPhase]=useState("hidden");
  ...
  // Curtain-rise: landing → app
  const launchApp=useCallback(()=>{
    setCurtainPhase("closing"); // curtains closed (visible)
    setTimeout(()=>{
      setScene("app");
      window.scrollTo(0,0);
      setCurtainPhase("open"); // curtains open (slide away)
      setTimeout(()=>setCurtainPhase("hidden"),800);
    },700);
  },[]);

  // Curtain-fall: app → landing
  ...
  // Curtain element (overlays everything during transitions)
  const curtainEl=(curtainPhase!=="hidden")&&(
    <div style={{position:'fixed',inset:0,zIndex:9999,pointerEvents:'none',display:'flex'}}>
      <div style={{flex:1,background:'linear-gradient(to bottom,#1a0a00,#0d0b11)',transition:'transform .7s cubic-bezier(.77,0,.18,1)',borderRight:'2px solid rgba(212,168,67,.4)',transform:curtainPhase==='open'?'translateX(-100%)':'translateX(0)'}}/>
      <div style={{flex:1,background:'linear-gradient(to bottom,#1a0a00,#0d0b11)',transition:'transform .7s cubic-bezier(.77,0,.18,1)',borderLeft:'2px solid rgba(212,168,67,.4)',transform:curtainPhase==='open'?'translateX(100%)':'translateX(0)'}}/>
    </div>
  );

  if(scene==="landing") return(
    <><style>{CSS}</style>{curtainEl}<LandingPage onLaunch={launchApp}/></>
  );
    ...
    <div className="app">
      <aside className={"side "+(desk?"":so?"ms":"mh")} style={desk?{position:"relative",transform:"none",opacity:1}:{}}>
        ...
        <nav className="nav">
          ...
          <div style={{marginTop:'auto',padding:'12px 9px 6px',borderTop:'1px solid var(--bd)'}}>
