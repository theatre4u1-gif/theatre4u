...
/* ── Landing Page ──────────────────────────────────────────────────────── */
.lp{background:var(--ink);min-height:100vh;color:var(--parchment);font-family:'DM Sans',sans-serif}
.lp-nav{position:fixed;top:0;left:0;right:0;z-index:100;display:flex;align-items:center;justify-content:space-between;padding:0 48px;height:68px;background:rgba(13,9,5,.85);backdrop-filter:blur(12px);border-bottom:1px solid rgba(196,118,26,.15)}
.lp-nav-logo{display:flex;align-items:center;gap:10px;font-family:'Abril Fatface',display;font-size:20px;color:var(--gold);text-decoration:none}
.lp-nav-links{display:flex;align-items:center;gap:6px}
.lp-hero{position:relative;min-height:100vh;display:flex;align-items:center;justify-content:center;text-align:center;overflow:hidden;padding:100px 24px 80px}
.lp-hero-bg{position:absolute;inset:0;background-size:cover;background-position:center;filter:brightness(.35)}
.lp-hero-over{position:absolute;inset:0;background:linear-gradient(180deg,rgba(13,9,5,.3) 0%,rgba(13,9,5,.7) 100%)}
.lp-hero-content{position:relative;z-index:2;max-width:860px}
.lp-kicker{display:inline-flex;align-items:center;gap:6px;background:rgba(196,118,26,.15);border:1px solid rgba(196,118,26,.3);border-radius:99px;padding:6px 14px;font-size:11.5px;font-weight:700;letter-spacing:1.5px;text-transform:uppercase;color:var(--gold);margin-bottom:28px}
.lp-h1{font-family:'Abril Fatface',display;font-size:clamp(42px,7vw,88px);line-height:1.05;color:var(--parchment);margin-bottom:24px}
.lp-h1 span{color:var(--gold)}
.lp-sub{font-size:clamp(16px,2.2vw,22px);color:rgba(253,246,236,.7);line-height:1.65;max-width:620px;margin:0 auto 44px}
.lp-cta-row{display:flex;align-items:center;justify-content:center;gap:14px;flex-wrap:wrap}
.lp-btn-p{background:linear-gradient(135deg,var(--gold),#a3621a);color:var(--ink);font-weight:800;font-size:15px;padding:15px 34px;border-radius:8px;border:none;cursor:pointer;transition:all .2s;font-family:'DM Sans',sans-serif;letter-spacing:.3px}
.lp-btn-p:hover{transform:translateY(-2px);box-shadow:0 8px 28px rgba(196,118,26,.4)}
.lp-btn-s{background:transparent;color:var(--parchment);font-weight:600;font-size:15px;padding:15px 28px;border-radius:8px;border:1px solid rgba(253,246,236,.25);cursor:pointer;transition:all .2s;font-family:'DM Sans',sans-serif}
.lp-btn-s:hover{border-color:var(--gold);color:var(--gold)}
.lp-feat-sec{padding:100px 24px;background:var(--ink)}
.lp-feat-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:28px;max-width:1080px;margin:56px auto 0}
.lp-feat-card{background:var(--bg2);border:1px solid var(--border);border-radius:14px;padding:36px 32px;transition:border-color .25s}
.lp-feat-card:hover{border-color:rgba(196,118,26,.4)}
.lp-feat-ico{font-size:36px;margin-bottom:18px}
.lp-feat-title{font-family:'Abril Fatface',display;font-size:19px;color:var(--parchment);margin-bottom:10px}
.lp-feat-desc{font-size:14px;color:rgba(253,246,236,.55);line-height:1.7}
.lp-testi-sec{padding:100px 24px;background:var(--bg2)}
.lp-testi-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:20px;max-width:1080px;margin:48px auto 0}
.lp-testi-card{background:var(--ink);border:1px solid var(--border);border-radius:12px;padding:28px;position:relative}
.lp-testi-card::before{content:'\u201C';font-family:'Abril Fatface',display;font-size:72px;color:rgba(196,118,26,.15);position:absolute;top:10px;left:20px;line-height:1}
.lp-testi-text{font-family:'Lora',serif;font-style:italic;font-size:16px;color:var(--parchment);line-height:1.7;margin-bottom:20px;position:relative;z-index:1;padding-top:20px}
.lp-testi-name{font-weight:700;font-size:13px;color:var(--gold)}
.lp-testi-role{font-size:12px;color:rgba(253,246,236,.45);margin-top:2px}
.lp-price-sec{padding:100px 24px;background:var(--ink)}
.lp-price-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:20px;max-width:960px;margin:56px auto 0}
.lp-price-card{background:var(--bg2);border:1px solid var(--border);border-radius:14px;padding:36px 28px;text-align:center;transition:border-color .25s,transform .25s}
.lp-price-card.featured{border-color:rgba(196,118,26,.5);background:linear-gradient(145deg,#1f140a,var(--bg2))}
.lp-price-card:hover{transform:translateY(-3px)}
.lp-price-plan{font-size:11px;text-transform:uppercase;letter-spacing:2px;color:var(--gold);font-weight:700;margin-bottom:12px}
.lp-price-amt{font-family:'Abril Fatface',display;font-size:48px;color:var(--parchment);margin-bottom:4px}
.lp-price-period{font-size:13px;color:rgba(253,246,236,.4);margin-bottom:28px}
.lp-price-feat{list-style:none;padding:0;margin:0 0 32px;text-align:left;display:flex;flex-direction:column;gap:9px}
.lp-price-feat li{font-size:13.5px;color:rgba(253,246,236,.7);display:flex;align-items:flex-start;gap:8px}
.lp-price-feat li::before{content:'✓';color:var(--gold);font-weight:700;flex-shrink:0}
.lp-cta-sec{background:var(--bg2);border-top:1px solid rgba(196,118,26,.1);text-align:center;padding:100px 24px;position:relative;overflow:hidden}
.lp-section-label{text-align:center;font-size:11px;text-transform:uppercase;letter-spacing:2.5px;color:var(--gold);font-weight:700;margin-bottom:16px}
.lp-section-title{text-align:center;font-family:'Abril Fatface',display;font-size:clamp(28px,4vw,46px);color:var(--parchment);margin-bottom:0}
.lp-footer{background:var(--ink);border-top:1px solid var(--border);padding:32px 48px;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:12px}
.lp-footer-copy{font-size:12px;color:rgba(253,246,236,.3)}
.lp-footer-links{display:flex;gap:20px}
.lp-footer-links button{background:none;border:none;color:rgba(253,246,236,.35);font-size:12px;cursor:pointer;font-family:'DM Sans',sans-serif;transition:color .2s}
.lp-footer-links button:hover{color:var(--gold)}
@media(max-width:900px){
  .lp-nav{padding:0 20px}
  .lp-feat-grid,.lp-testi-grid,.lp-price-grid{grid-template-columns:1fr}
...
// ── Landing Page ──────────────────────────────────────────────────────────────
function LandingPage({ onGetStarted, onSignIn, showAuth, onAuth }) {
  const FEATURES = [
    { ico:"📦", title:"Complete Inventory", desc:"Track every costume, prop, lighting rig, and piece of audio equipment. Add photos, condition notes, storage locations, and QR labels." },
    { ico:"🔲", title:"QR Code Labels", desc:"Every item gets a printable QR label. Scan it with any phone to instantly pull up full details — no app required." },
    { ico:"🏪", title:"Theatre Marketplace", desc:"List items for rent or sale. Connect with other schools and community theatres to borrow what you need and earn from what you have." },
    { ico:"📊", title:"Reports & Export", desc:"Condition breakdowns, availability reports, and one-click CSV export. Always know exactly what you own and what it's worth." },
    { ico:"📸", title:"Photo Documentation", desc:"Upload up to 5 photos per item. Visual records make it easy to verify condition, find items in storage, and settle disputes." },
    { ico:"🔒", title:"Secure & Private", desc:"Each organisation sees only their own data. Powered by Supabase with row-level security. Your inventory stays yours." },
  ];
  const TESTIMONIALS = [
    { text:"We used to track everything in a spreadsheet that only one person could update. Theatre4u changed everything — our whole team can see and update the inventory in real time.", name:"Sarah M.", role:"Drama Director, Lincoln High School" },
    { text:"The QR labels alone are worth it. I can scan a bin and immediately see every item inside, when it was last used, and its condition. No more mystery boxes backstage.", name:"James T.", role:"Technical Director, Riverside Community Theatre" },
    { text:"We rented out our fog machine and two wireless mics to another school last semester and covered our subscription cost for the whole year. The marketplace is a gem.", name:"Patricia K.", role:"Performing Arts Coordinator, District 47" },
  ];
  const PLANS = [
    { plan:"Starter", price:"$9", period:"/month", features:["Up to 200 inventory items","Marketplace listings","Photo uploads","QR code labels","CSV export"], featured:false },
    { plan:"Pro", price:"$19", period:"/month", features:["Unlimited inventory items","Priority marketplace placement","Advanced reports","Email support","Everything in Starter"], featured:true },
    { plan:"District", price:"$49", period:"/month", features:["Multiple organisations","District-wide dashboard","Bulk import","Dedicated support","Everything in Pro"], featured:false },
  ];
  ...
  return (
    <div className="lp">
      ...
      <nav className="lp-nav">
        <div className="lp-nav-logo">🎭 Theatre4u</div>
        <div className="lp-nav-links">
          <button className="lp-btn-s" style={{padding:"9px 20px",fontSize:13}} onClick={onSignIn}>Sign In</button>
          <button className="lp-btn-p" style={{padding:"9px 20px",fontSize:13}} onClick={onGetStarted}>Get Started Free</button>
        </div>
      </nav>

      {/* Hero */}
      <section className="lp-hero">
        <div className="lp-hero-bg" style={{backgroundImage:`url(https://images.unsplash.com/photo-1503095396549-807759245b35?w=1800&h=900&fit=crop&auto=format&q=85)`}}/>
        <div className="lp-hero-over"/>
        <div className="lp-hero-content">
          <div className="lp-kicker">🎭 Built for Theatre Programs</div>
          <h1 className="lp-h1">The Inventory &amp; Marketplace<br/>for <span>Theatre Programs</span></h1>
          <p className="lp-sub">Track every costume, prop, and piece of equipment your program owns. List items for rent or sale. Generate QR labels for every storage bin. All in one place.</p>
          <div className="lp-cta-row">
            <button className="lp-btn-p" onClick={onGetStarted}>Start for Free — No Card Required</button>
            <button className="lp-btn-s" onClick={onSignIn}>Sign In to Your Account</button>
          </div>
        </div>
      </section>

      {/* Features */}
      <section className="lp-feat-sec">
        <div className="lp-section-label">Everything You Need</div>
        <h2 className="lp-section-title">Built for the way theatre programs actually work</h2>
      ...
      <section className="lp-testi-sec">
        <div className="lp-section-label">What Directors Say</div>
        <h2 className="lp-section-title">Trusted by theatre programs everywhere</h2>
      ...
      {/* Pricing */}
      <section className="lp-price-sec">
        ...
        <div className="lp-price-grid">
          {PLANS.map(p=>(
            <div key={p.plan} className={"lp-price-card"+(p.featured?" featured":"")}>
              {p.featured && <div style={{fontSize:10,fontWeight:700,letterSpacing:1.5,textTransform:"uppercase",color:"var(--gold)",marginBottom:8}}>Most Popular</div>}
              <div className="lp-price-plan">{p.plan}</div>
              <div className="lp-price-amt">{p.price}</div>
              <div className="lp-price-period">{p.period}</div>
              <ul className="lp-price-feat">{p.features.map(f=><li key={f}>{f}</li>)}</ul>
              <button className={p.featured?"lp-btn-p":"lp-btn-s"} style={{width:"100%",padding:"13px 0"}} onClick={onGetStarted}>Get Started</button>
            </div>
          ))}
        </div>
        <p style={{textAlign:"center",marginTop:28,fontSize:13,color:"rgba(253,246,236,.4)"}}>14-day free trial on all plans. Cancel anytime. No credit card required to start.</p>
      </section>

      {/* CTA */}
      <section className="lp-cta-sec">
        <div className="lp-section-label">Get Started Today</div>
        <h2 className="lp-section-title" style={{marginBottom:24}}>Your programme deserves better than a spreadsheet.</h2>
        <p style={{color:"rgba(253,246,236,.55)",fontSize:16,marginBottom:40,maxWidth:500,margin:"0 auto 40px"}}>Join theatre programs across the country using Theatre4u to manage their inventory and connect with other organisations.</p>
        <button className="lp-btn-p" style={{fontSize:16,padding:"17px 42px"}} onClick={onGetStarted}>Create Your Free Account</button>
      </section>

      {/* Footer */}
      <footer className="lp-footer">
        <div className="lp-footer-copy">© {new Date().getFullYear()} Theatre4u. All rights reserved.</div>
      ...
      {/* Auth overlay */}
      {showAuth && (
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.75)",zIndex:500,display:"flex",alignItems:"center",justifyContent:"center",padding:20}} onClick={e=>e.target===e.currentTarget&&onGetStarted()}>
...
export default function App() {
  ...
  if(!user) return <LandingPage onGetStarted={()=>setShowAuth("signup")} onSignIn={()=>setShowAuth("login")} showAuth={showAuth} onAuth={u=>{setUser(u);setShowAuth(null);}}/>;
