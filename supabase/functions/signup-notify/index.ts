...
function teamWelcomeHtml(orgName:string,role:string):string{
  ...
  const roleDesc=role==='co_director'?'Full access — same as the director. You can add, edit, and manage everything in the program.':role==='stage_manager'?'You can add and edit inventory items, access the Funding Tracker, Backstage Exchange, and Community Board.':role==='crew'?'You can add and edit items and upload photos to the inventory.':'You can view and search the program\'s inventory.';
  return`<!DOCTYPE html><html><head><meta charset="utf-8"></head><body style="margin:0;padding:0;background:#f5f0e8;font-family:Arial,sans-serif"><div style="max-width:520px;margin:0 auto;background:#fff"><div style="background:#1a1200;padding:20px 28px;text-align:center"><span style="font-size:36px">&#x1F3AD;</span><div style="font-family:Georgia,serif;font-size:22px;font-weight:700;color:#d4a843;margin-top:6px">Theatre4u™</div></div><div style="padding:28px"><h2 style="font-family:Georgia,serif;color:#1a1200;font-size:22px;margin:0 0 10px">You’re in!</h2><p style="color:#444;font-size:15px;line-height:1.7;margin:0 0 18px">You’ve been added to <strong>${orgName}</strong>’s backstage team as <strong>${roleLabel}</strong>.</p><div style="background:#f5f0e8;border-radius:8px;padding:14px;font-size:13px;color:#666;margin-bottom:22px;line-height:1.6"><strong>Your access:</strong> ${roleDesc}</div><div style="text-align:center;margin-bottom:24px"><a href="https://theatre4u.org" style="display:inline-block;background:#d4a843;color:#1a0f00;padding:14px 32px;border-radius:8px;text-decoration:none;font-weight:700;font-size:15px">Open Theatre4u →</a></div><p style="color:#888;font-size:13px;line-height:1.6">Sign in with the email and password you used to join. You’ll see ${orgName}’s inventory when you log in.</p></div><div style="padding:12px 24px;border-top:1px solid #e8e0d0;text-align:center;font-size:11px;color:#aaa">Theatre4u™ · Artstracker LLC · <a href="mailto:hello@theatre4u.org" style="color:#aaa">hello@theatre4u.org</a></div></div></body></html>`;
}
...
Deno.serve(async(req:Request)=>{
  ...
  try{
    ...
    const{org_id,org_email,is_team_member,team_org_name,team_role}=body;
    ...
    if(is_team_member){
      ...
      const orgName=team_org_name||'your theatre program';
      const role=team_role||'crew';

      // Send you're-in email to team member
      const teamRes=await fetch('https://api.resend.com/emails',{
        ...
        body:JSON.stringify({
          ...
          to:[toEmail],
          subject:`You're in — ${orgName} on Theatre4u™`,
          html:teamWelcomeHtml(orgName,role),
        }),
      });
